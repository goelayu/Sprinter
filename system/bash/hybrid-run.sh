...
cat $sites | while read i; do
    cat $2/static/$i  >> $3/$LOGFILE/static/urls.txt
done
cd $WPRDIR && GOROOT=$GOROOT time go run src/static/crawler* -az $AZPORT -wpr $WPRDATA -n 50 -v \
-proxy $3/$LOGFILE/static  -pages $3/$LOGFILE/static/urls.txt -azlog $3/$LOGFILE/output/az.log &> $3/$LOGFILE/output/static.log
